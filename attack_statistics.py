...
def save_template_example():
    """保存模板示例文件"""
    # 使用简化占位符的新模板
    template_content = """某某防火墙共监测到{防火墙攻击次数}次攻击，某某探针共监测{探针攻击ip数}个IP的{探针攻击次数}次攻击，分析了{探针攻击次数}条告警日志，分析后进行了{高风险ip数}次封堵，未发现攻击成功事件，并根据威胁情报封禁了0个IP。"""
...
def main():
    ...
    if args.template and os.path.exists(args.template):
        ...
        # 使用默认模板（简化占位符）
        template = """某某防火墙共监测到{防火墙攻击次数}次攻击，某某探针共监测{探针攻击ip数}个IP的{探针攻击次数}次攻击，分析了{探针攻击次数}条告警日志，分析后进行了{高风险ip数}次封堵，未发现攻击成功事件，并根据威胁情报封禁了0个IP。"""
